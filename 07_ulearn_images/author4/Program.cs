...
namespace Recognizer
{
    internal static class Program
    {
        ...
        private static void Main()
        {
            ...
            var grayscale = GrayscaleTask.TransformToGray(pixels);
            ...
            var trashhold = ThresholdFilterTask.FilterByThreshold(sobell, 0.1);
            panel.Controls.Add(CreateBox(ConvertToBitmap(trashhold)), 1, 1);

            var bitmap = ConvertToBitmap(sobell);
            using (var g = Graphics.FromImage(bitmap))
            {
                var lines = HoughTransformTask.HoughAlgo(sobell);
