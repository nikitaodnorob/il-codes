...
namespace Antiplagiarism
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var folder = Folders.StrangeSource;
            if (args.Length != 0)
            {
                folder = new DirectoryInfo(args[0]);
            }
            var documents = DocumentLoader.LoadAllSituationsNames(folder)
        ...
        {
            return 2 * comparisonResult.Distance_ / (comparisonResult.Document1.Count + comparisonResult.Document2.Count);
        }
